...
pub fn random_vectorized_field_tests<F: VectorizedField + Serde>(type_name: String) {
    let mut rng = test_rng();

    random_serdes_tests::<F, _>(&mut rng, type_name);
}

fn random_serdes_tests<F: VectorizedField + Serde, R: RngCore>(mut rng: R, _type_name: String) {
